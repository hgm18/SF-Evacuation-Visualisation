...
const m = 1;//mass
const desired_speed = 3;//mean desired speed
...
function showPedForceVect(Fx, Fy, posx, posy) {
    let i = Fx;
    let j = Fy;
    let x = posx;
    let y = posy;
...
function setup() {
    ...
    reset.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight);
    ...
    start.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight - 100);
    pedInput = createInput(1, 'number');
    pedInput.style("font-size", "22px");
    pedInput.size(buttonWidth, 30);
    pedInput.min = '1';
    pedInput.max = '100';
    pedInput.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight - 200);
    pedNoTitle = createDiv('No. of Pedestrians:');
    pedNoTitle.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - 275);
    pedNoTitle.html('No. of Pedestrians:');
    showForces = createButton("Show/Hide Forces");
    showForces.mousePressed(showForcesClick);
    showForces.style("background", "#0E406F");
    showForces.style("color", "#FFFFFF");
    showForces.style("font-size", "28px");
    showForces.style("opacity", "0.9");
    showForces.style("transition", "0.05s");
    showForces.size(buttonWidth, buttonHeight+20);
    showForces.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight-355);
}
function showForcesClick(){
...
function startClick() {
    if (exitPlaced) {
        startMode = true;
        const pedNum = pedInput.value();
        r.generatePed(pedNum);
    }
}
function mousePressed() {
    let x = (mouseX - canvasAdjust) * r.width / canvasWidth;
    let y = (mouseY - canvasAdjust) * r.height / canvasHeight;
    let buf = 0.05 * roomWidth;
    let w;
    let nearWall = false;
    let vertical;
    for (let i = 0; i < r.walls.length; i++) {//Finding walls
        if (r.walls[i].startX == r.walls[i].endX) {//Vertical Wall
            if (x < r.walls[i].startX + buf && x > r.walls[i].startX - buf) {//Near X of wall
                if (y < r.walls[i].endY + buf && y > r.walls[i].startY - buf) {//Near Y of wall
                    nearWall = true;
                    w = r.walls[i];
                    vertical = true;
                    break;
        ...
        else if (r.walls[i].startY == r.walls[i].endY) {//Horizontal Wall
            if (y < r.walls[i].startY + buf && y > r.walls[i].startY - buf) {//Near X of wall
                if (x < r.walls[i].endX + buf && x > r.walls[i].startX - buf) {//Near Y of wall
                    nearWall = true;
                    w = r.walls[i];
                    vertical = false;
                    break;
    ...
    if (nearWall) {
        if (vertical) {
            if (y > 0.95 * roomHeight) {
                r.create1Exit(w.startX, w.endX, 0.9 * roomHeight, roomHeight);
            }
            else if (y < 0.05 * roomHeight) {
                r.create1Exit(w.startX, w.endX, 0, 0.1 * roomHeight);
            }
            else {
                r.create1Exit(w.startX, w.endX, y - 0.05 * roomHeight, y + 0.05 * roomHeight);
            }
        }
        else {
            if (x > 0.95 * roomWidth) {
                r.create1Exit(0.9 * roomWidth, roomWidth, w.startY, w.endY);
            }
            else if (x < 0.05 * roomWidth) {
                r.create1Exit(0, 0.1 * roomWidth, w.startY, w.endY);
            }
            else {
                r.create1Exit(x - 0.05 * roomWidth, x + 0.05 * roomWidth, w.startY, w.endY);
            }
        }
        r.breaksInWalls();
        exitPlaced = true;
...
function draw() {
    ...
    if (forceStatus){
        for (let i = 0; i < r.pedestrians.length; i++) {
            ...
            Fx = (r.pedestrians[i].border_force[0])*((canvasWidth+2*canvasAdjust)/r.width);
            Fy = (r.pedestrians[i].border_force[1])*((canvasHeight+2*canvasAdjust)/r.height);
            showBorderForceVect(Fx,Fy,posx,posy);
            Fx = r.pedestrians[i].pedestrian_force[0] * canvasWidth / r.width;
            Fy = r.pedestrians[i].pedestrian_force[1] * canvasHeight / r.height;
            showPedForceVect(Fx, Fy, posx, posy);
        }
    }
