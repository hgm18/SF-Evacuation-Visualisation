...
const m = 1;//mass
const desired_speed = 4;//mean desired speed
...
r.generateWall();
r.generatePed(1);
r.create1Exit(0.35*roomWidth,0.65*roomWidth,0,0);
r.breaksInWalls();
...
function setup() {
    ...
    verticalWallMode = false;
    verticalWall = createButton("Place Vertical Wall");
    verticalWall.mousePressed(vClick);
    verticalWall.style("background", "#0E406F");
    verticalWall.size(buttonWidth, buttonHeight*1.5);
    verticalWall.style("color", "#FFFFFF");
    verticalWall.style("font-size", "28px");
    verticalWall.style("opacity", "0.9");
    verticalWall.style("transition", "0.05s");
    verticalWall.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight-200);
    horizontalWallMode = false;
    horizontalWall = createButton("Place Horizontal Wall");
    horizontalWall.mousePressed(hClick);
    horizontalWall.style("background", "#0E406F");
    horizontalWall.size(buttonWidth, buttonHeight*2);
    horizontalWall.style("color", "#FFFFFF");
    horizontalWall.style("font-size", "28px");
    horizontalWall.style("opacity", "0.9");
    horizontalWall.style("transition", "0.05s");
    horizontalWall.position(canvasWidth + 2 * canvasAdjust + 50, canvasHeight - buttonHeight-320);
}
function resetClick() {
...
function hClick() {
    if (!startMode){
        verticalWallMode = false;
        horizontalWallMode = true;
    }
}
function mousePressed () {
    if (verticalWallMode && mouseX>canvasAdjust && mouseX<canvasWidth+canvasAdjust && mouseY>canvasAdjust &&mouseY<canvasHeight+canvasAdjust){
        let x = (mouseX - canvasAdjust)*r.width/canvasWidth;
        let y = (mouseY - canvasAdjust)*r.height/canvasHeight;
        r.create1Wall(x,x,y-0.05*roomHeight,y+0.05*roomHeight);
        r.step();
    }
    else if(horizontalWallMode&& mouseX>canvasAdjust && mouseX<canvasWidth+canvasAdjust && mouseY>canvasAdjust &&mouseY<canvasHeight+canvasAdjust){
        let x = (mouseX - canvasAdjust)*r.width/canvasWidth;
        let y = (mouseY - canvasAdjust)*r.height/canvasHeight;
        r.create1Wall(x-0.05*roomWidth,x+0.05*roomWidth,y,y);
        r.step();
    }
}
...
function draw() {
    ...
    strokeWeight(6);
    if (verticalWallMode){
        line(mouseX,mouseY-0.05*canvasHeight,mouseX,mouseY+0.05*canvasHeight);
    }
    else if(horizontalWallMode){
        line(mouseX-0.05*canvasWidth,mouseY,mouseX+0.05*canvasWidth,mouseY);
    }
    ...
    for (let i = 0; i < r.pedestrians.length; i++) {
        ...
        let Fx = r.pedestrians[i].border_force[0] * canvasWidth / r.width;
        let Fy = r.pedestrians[i].border_force[1] * canvasHeight / r.height;
